...
import { type Order, type OrderStatus, type CheckoutItem } from "@/features/orders/types/order.types";
...
export function TableOrderPage({ tableId }: { tableId: string }) {
  const { user } = useAuth();
  ...
  function place() {
    ...
    if (!user) {
      toast.error("Please sign in before sending an order to the kitchen.");
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <CheckoutModal
        ...
        onPaid={(payment) => {
          setPaid(true);
          const payments = loadStored<Record<string, unknown>[]>("smartserve:demo-payments", []);
          saveStored("smartserve:demo-payments", [
            { orderId: order.id, table: tableLabel, paidAt: new Date().toISOString(), ...payment },
            ...payments,
          ]);
